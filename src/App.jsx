...
// TAB譜と同じ：上=1弦e（高音）、下=6弦E（低音）
// 下からEADGBe → 画面上では逆順で描画
const STRING_TUNING = [4,11,7,2,9,4]; // 上から: 1e,2B,3G,4D,5A,6E
const STRING_LABELS = ["e","B","G","D","A","E"]; // 上=1弦、下=6弦
const FRET_COUNT = 12;
function getNoteAtFret(s,f){return(STRING_TUNING[s]+f)%12;}

// ── Web Audio: ギター風音源 ──────────────────────────
// Karplus-Strong 弦モデル + 軽いリバーブ
...
// ノート番号 → Hz (A4=440, MIDI式)
function noteToHz(noteIdx, octave = 4) {
  // noteIdx: 0=C, 1=C#, ...11=B
  const midi = (octave + 1) * 12 + noteIdx;
  return 440 * Math.pow(2, (midi - 69) / 12);
}

// Karplus-Strong で1音 (ギター弦シミュレーション)
function playString(ctx, freq, vol = 0.5, startTime = 0) {
  ...
  // Karplus-Strong: 最初のN サンプルに雑音、残りは低域フィルタで減衰
  const ksBuffer = new Float32Array(N);
  for (let i = 0; i < N; i++) ksBuffer[i] = Math.random() * 2 - 1;
  for (let i = 0; i < bufLen; i++) {
    ...
    if (i < N) {
      ...
      // ローパスフィルタ（平均化）＋減衰係数
      const decay = 0.996;
      ksBuffer[idx] = decay * 0.5 * (ksBuffer[idx] + ksBuffer[nextIdx]);
      data[i] = ksBuffer[idx];
    }
  }
...
function playNote(noteIdx, octave = 3, vol = 0.5, delay = 0) {
  try {
    ...
    const freq = noteToHz(noteIdx, octave);
    playString(ctx, freq, vol, ctx.currentTime + delay);
  } catch(e) { console.warn("audio error", e); }
}

// コードを鳴らす（アルペジオ or ストローク）
function playChord(root, type, mode = "strum") {
  try {
    ...
    if (mode === "strum") {
      // ストローク: 各音を少しずらして
      notes.forEach((n, i) => {
        playString(ctx, noteToHz(n, n < rootIdx ? 3 : 3), 0.4, ctx.currentTime + i * 0.04);
      });
    } else {
      // アルペジオ
      notes.forEach((n, i) => {
        playString(ctx, noteToHz(n, 3), 0.45, ctx.currentTime + i * 0.18);
      });
    }
  } catch(e) { console.warn("audio error", e); }
}
...
function pickWeightedChord(stats){
  ...
  for(let ri=0;ri<12;ri++){
    for(const type of typeKeys){
      ...
      const weight=Math.max(1,Math.round((1-acc)*4+1));
      candidates.push({rootIdx:ri,type,weight});
    }
  }
...
function PlayButton({onClick, label="▶", size="sm", style={}}) {
  ...
  return (
    <button onClick={handleClick} style={{
      ...base,cursor:"pointer",border:"none",
      background: active ? "#52c97a" : "#1e3a1e",
      color: active ? "#fff" : "#52c97a",
      transition:"all 0.15s",fontWeight:600,...style
    }}>{label}</button>
  );
}
...
function FretboardDiagram({root,intervals,colorMap,onNoteClick}){
  ...
  return (
    <div style={{overflowX:"auto",marginTop:8}}>
      <svg width={Math.max(500,FC*36+60)} height={160} style={{display:"block"}}>
        {Array.from({length:FC+1},(_,f)=><text key={f} x={60+f*36-10} y={14} fontSize={11} fill="#888" textAnchor="middle">{f}</text>)}
        <rect x={60} y={20} width={3} height={120} fill="#aaa"/>
        {Array.from({length:FC},(_,f)=><rect key={f} x={60+(f+1)*36} y={20} width={1} height={120} fill="#444"/>)}
        {STRING_TUNING.map((_,s)=><line key={s} x1={60} y1={28+s*22} x2={60+FC*36} y2={28+s*22} stroke="#666" strokeWidth={1.5-s*0.15}/>)}
        {STRING_LABELS.map((lbl,s)=><text key={s} x={46} y={32+s*22} fontSize={12} fill="#aaa" textAnchor="middle">{lbl}</text>)}
        {STRING_TUNING.map((_,s)=>Array.from({length:FC+1},(_,f)=>{
          ...
          return (
            <g key={`${s}-${f}`} onClick={()=>onNoteClick&&onNoteClick(note)} style={{cursor:onNoteClick?"pointer":"default"}}>
              <circle cx={60+f*36} cy={28+s*22} r={10} fill={color} opacity={0.9}/>
              <text x={60+f*36} y={32+s*22} fontSize={9} fill="white" textAnchor="middle" fontWeight="bold">
                {NOTES[(rootIdx+iv)%12]}
              </text>
            </g>
          );
...
function FretboardFull({root,type,highlightInterval,onNoteClick}){
  ...
  const chordTones=intervals.map(i=>(rootIdx+i)%12);
  const w=FRET_COUNT*36+72,h=6*26+36;
  return (
    <div style={{overflowX:"auto"}}>
      <svg width={w} height={h} style={{display:"block",minWidth:w}}>
        {Array.from({length:FRET_COUNT+1},(_,f)=><text key={f} x={62+f*36} y={13} fontSize={9} fill="#444" textAnchor="middle">{f}</text>)}
        <rect x={62} y={18} width={3} height={h-30} fill="#777" rx={1}/>
        {Array.from({length:FRET_COUNT},(_,f)=><rect key={f} x={62+(f+1)*36} y={18} width={1} height={h-30} fill="#222"/>)}
        {STRING_TUNING.map((_,s)=><line key={s} x1={62} y1={26+s*26} x2={62+FRET_COUNT*36} y2={26+s*26} stroke="#2a2a2a" strokeWidth={1}/>)}
        ...
        {STRING_TUNING.map((_,s)=>Array.from({length:FRET_COUNT+1},(_,f)=>{
          ...
          return (
            <g key={`${s}-${f}`} onClick={()=>onNoteClick&&onNoteClick(note,s)} style={{cursor:onNoteClick?"pointer":"default"}}>
              <circle cx={62+f*36} cy={26+s*26} r={10} fill={color} opacity={lit?0.95:0.18}/>
              <text x={62+f*36} y={30+s*26} fontSize={8.5} fill={lit?"white":"#444"} textAnchor="middle" fontWeight="bold">{INTERVAL_LABEL[iv]}</text>
            </g>
          );
        }))}
      </svg>
...
function VisualizeMode(){
  ...
  return (
    <div>
      ...
      <div style={{fontSize:11,color:"#555",marginBottom:16}}>↑ タップして音を確認</div>
      {/* コード再生ボタン */}
      ...
      <div style={{display:"flex",gap:16,flexWrap:"wrap",marginBottom:24}}>
        <div style={{background:"#1a1a1a",borderRadius:8,padding:16,flex:"1 1 240px"}}>
        ...
        <div style={{background:"#1a1a1a",borderRadius:8,padding:16,flex:"2 1 360px"}}>
...
function StatsPanel({stats,onReset}){
  ...
  if(entries.length===0)return(
    <div style={{padding:"16px",background:"#1a1a1a",borderRadius:10,color:"#555",fontSize:13,textAlign:"center"}}>まだデータなし。クイズを解くと苦手コードが表示されます。</div>
  );
  return (
...
function QuizMode({stats,record,resetStats}){
  ...
  const nextQ=useCallback(()=>{
    ...
    if(quizType==="context"){q=generateContextQ();}
      ...
      if(Math.random()<0.6){const p=pickWeightedChord(stats);q=generateSingleQ(p.rootIdx,p.type);}
      else{q=generateContextQ();}
    }
    setQuestion(q);setSelected(null);
    // 問題のコードをストロークで再生
    setTimeout(()=>{
      try{ playChord(q.root, q.type, "strum"); }catch(e){}
    }, 100);
  },[quizType,stats]);
  ...
  const handleAnswer=(note)=>{
    ...
    // 正解音を鳴らす
    const noteIdx=NOTES.indexOf(question.correctNote);
    playNote(noteIdx,3,0.55,0);
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap",alignItems:"center"}}>
        <div style={{background:"#1a1a1a",borderRadius:8,padding:"7px 14px",textAlign:"center"}}>
          <div style={{fontSize:20,fontWeight:800,color:"#f4a535"}}>{accuracy}%</div>
          <div style={{fontSize:10,color:"#555"}}>正答率</div>
        </div>
        <div style={{background:"#1a1a1a",borderRadius:8,padding:"7px 14px",textAlign:"center"}}>
          <div style={{fontSize:20,fontWeight:800,color:"#4a9eff"}}>{score.correct}/{score.total}</div>
          <div style={{fontSize:10,color:"#555"}}>正解/出題</div>
        </div>
        <div style={{background:"#1a1a1a",borderRadius:8,padding:"7px 14px",textAlign:"center"}}>
          <div style={{fontSize:20,fontWeight:800,color:streak>=3?"#52c97a":"#ddd"}}>🔥{streak}</div>
          <div style={{fontSize:10,color:"#555"}}>連続</div>
        </div>
        <div style={{display:"flex",gap:3,alignItems:"center",flexWrap:"wrap"}}>
          {history.map((h,i)=><div key={i} style={{width:7,height:7,borderRadius:"50%",background:h.correct?"#52c97a":"#e05252",opacity:0.3+(i/history.length)*0.7}}/>)}
        </div>
        <button onClick={()=>setShowStats(s=>!s)} style={{marginLeft:"auto",fontSize:11,color:showStats?"#4a9eff":"#555",background:"none",border:"1px solid "+(showStats?"#4a9eff":"#333"),borderRadius:4,padding:"4px 10px",cursor:"pointer"}}>
          {showStats?"▲ 閉じる":"📊 苦手分析"}
        </button>
      </div>
      ...
      <div style={{background:"#111",border:"1px solid #2a2a2a",borderRadius:12,padding:"18px 18px 14px",marginBottom:14}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:4}}>
          <div>
            ...
            <div style={{fontSize:26,fontWeight:800,color:"#fff",letterSpacing:"-0.5px"}}>
              {question.chordLabel}
              <span style={{fontSize:13,color:"#555",fontWeight:400,marginLeft:10}}>({CHORD_TYPES[question.type].label})</span>
            </div>
          </div>
          {/* 再生ボタン */}
        ...
        <div style={{fontSize:14,color:"#aaa",marginBottom:14}}>
          の <span style={{color:INTERVAL_COLOR[question.targetIv],fontWeight:700,fontSize:17}}>{question.intervalLabel}</span> はどの音？
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          {question.choices.map(note=>{
            ...
            return (
              <button key={note} onClick={()=>handleAnswer(note)} style={{padding:"13px",borderRadius:8,fontSize:19,fontWeight:700,background:bg,border:`1px solid ${border}`,color,cursor:selected!==null?"default":"pointer",transition:"all 0.15s"}}>
                {note}{selected!==null&&note===question.correctNote&&<span style={{fontSize:10,fontWeight:400,marginLeft:6}}>✓</span>}
              </button>
            );
          })}
        </div>
        {selected!==null&&(
          <div style={{marginTop:12,padding:"10px 13px",borderRadius:8,background:selected===question.correctNote?"#0d2a0d":"#2a0d0d",borderLeft:`3px solid ${selected===question.correctNote?"#52c97a":"#e05252"}`}}>
            <div style={{fontSize:13,color:selected===question.correctNote?"#52c97a":"#e05252",fontWeight:600,marginBottom:5}}>
              {selected===question.correctNote?"✓ 正解！":`✗ 不正解。正解は ${question.correctNote}`}
            </div>
            <div style={{fontSize:12,color:"#888",display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
              ...
              {intervals.map(iv=>{
                ...
                return (
                  <span key={iv} onClick={()=>playNote((rootIdx+iv)%12,3,0.5)} style={{color:INTERVAL_COLOR[iv],fontWeight:600,cursor:"pointer",textDecoration:"underline",textDecorationStyle:"dotted"}}>
                    {n}<span style={{color:"#555",fontSize:9,marginLeft:2}}>({INTERVAL_LABEL[iv]})</span>
                  </span>
                );
              })}
      ...
      <button onClick={nextQ} style={{width:"100%",padding:"13px",borderRadius:8,fontSize:14,fontWeight:700,background:"#4a9eff",color:"#fff",border:"none",cursor:"pointer"}}>
        {selected!==null?"次の問題 →":"スキップ"}
      </button>
    </div>
  );
}
    ...
    <div style={{background:"#0d1a0d",border:"1px solid #1a2a1a",borderRadius:10,padding:"12px 16px"}}>
      ...
      {common.length>0?(
        <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
          {common.map(n=>(
            <span key={n} onClick={()=>playNote(NOTES.indexOf(n),3,0.5)} style={{padding:"4px 10px",borderRadius:5,background:"#1a3a1a",color:"#52c97a",fontWeight:700,fontSize:14,cursor:"pointer"}}>
              {n}
            </span>
          ))}
...
function ProgressionMode(){
  ...
  // コードボタンを押したら発音
  const handleChordSelect=(i)=>{
    setActiveChord(i);setHighlightInterval(null);
    playChord(progression.chords[i].root,progression.chords[i].type,"strum");
  };

  // 進行を順番に自動再生
  const playAll=()=>{
    progression.chords.forEach((c,i)=>{
      setTimeout(()=>playChord(c.root,c.type,"strum"), i*900);
    });
  };
  ...
  return (
    <div>
      ...
      {/* 進行一括再生 */}
      <div style={{marginBottom:16}}>
        <PlayButton label={`▶ ${progression.name} を通しで聴く`} size="md" onClick={playAll}/>
      </div>
...
export default function App(){
  ...
  return (
    <div style={{background:"#111",color:"#ddd",minHeight:"100vh",fontFamily:"'Helvetica Neue',sans-serif",padding:"20px 16px",maxWidth:760,margin:"0 auto"}}>
